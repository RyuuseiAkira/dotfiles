...
export default () => {
    ...
    const Window = ({ address, at: [x, y], size: [w, h], workspace: { id, name }, class: c, title, xwayland }, screenCoords) => {
        ...
        const appIcon = Widget.Icon({
            icon: substitute(c),
            size: Math.min(w, h) * userOptions.overview.scale / 2,
        });
        return Widget.Button({
            ...
            vpack: 'start',
            css: `
                margin-left: ${Math.min(x * userOptions.overview.scale)}px;
                margin-top: ${Math.min(y * userOptions.overview.scale)}px;
                margin-right: -${Math.min((x + w) * userOptions.overview.scale)}px;
                margin-bottom: -${Math.min((y + h) * userOptions.overview.scale)}px;
            `,
            onClicked: (self) => {
    ...
    const Workspace = (index) => {
        ...
        const fixed = Widget.Box({
            attribute: {
                put: (widget, x, y) => {
                    ...
                    const newCss = `
                        margin-left: ${Math.round(x)+3}px;
                        margin-top: ${Math.round(y)+3}px;
                        margin-right: -${Math.round(x + (widget.attribute.w * userOptions.overview.scale))}px;
                        margin-bottom: -${Math.min(y + (widget.attribute.h * userOptions.overview.scale))}px;
                    `;
                    widget.css = newCss;
                    fixed.pack_start(widget,0,0,0);
                },
                move: (widget, x, y) => {
                    if (!widget) return;
                    if (!widget.attribute) return;
        ...
        const WorkspaceNumber = ({ index, ...rest }) => Widget.Label({
            ...
            label: `${index}`,
            css: `
                margin: ${Math.min(SCREEN_WIDTH, SCREEN_HEIGHT) * userOptions.overview.scale * userOptions.overview.wsNumMarginScale+2}px;
                font-size: ${SCREEN_HEIGHT * userOptions.overview.scale * userOptions.overview.wsNumScale+2}px;
            `,
