...
if [ "${enableWallDcol}" -eq 0 ]; then
    colorScheme="$({ grep -q "^[[:space:]]*\$COLOR[-_]SCHEME\s*=" "${hyprThemeDir}/userprefs.conf" && grep "^[[:space:]]*\$COLOR[-_]SCHEME\s*=" "${hyprThemeDir}/userprefs.conf" | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' ;} || 
                        grep 'gsettings set org.gnome.desktop.interface color-scheme' "${hyprThemeDir}/userprefs.conf" | awk -F "'" '{print $((NF - 1))}')"
