...
gtkTheme="$(
{ grep -q "^[[:space:]]*\$GTK[-_]THEME\s*=" "${hyprThemeDir}/userprefs.conf" && grep "^[[:space:]]*\$GTK[-_]THEME\s*=" "${hyprThemeDir}/userprefs.conf" | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' ;} || 
grep 'gsettings set org.gnome.desktop.interface gtk-theme' "${hyprThemeDir}/userprefs.conf" | awk -F "'" '{print $((NF - 1))}'
)"

gtkIcon="$(
{ grep -q "^[[:space:]]*\$ICON[-_]THEME\s*=" "${hyprThemeDir}/userprefs.conf" && grep "^[[:space:]]*\$ICON[-_]THEME\s*=" "${hyprThemeDir}/userprefs.conf" | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' ;} ||  
grep 'gsettings set org.gnome.desktop.interface icon-theme' "${hyprThemeDir}/userprefs.conf" | awk -F "'" '{print $(NF - 1)}'
)"
