...
function start_blinking() {
  if is_webcam_on; then
    ...
    notify-send -a "t2" -r 91190 -t 800 -i "/home/akira/.config/dunst/icons/cam-on.svg" "Camera on"
    return 0
  else
    echo "Webcam is off"
    ...
    if [ -f "$PID_FILE" ]; then
      ...
      notify-send -a "t2" -r 91190 -t 800 -i "/home/akira/.config/dunst/icons/cam-off.svg" "Camera off"
    fi
    return 1
  fi
}
